...
- **`expected_status` semantic — enforced whenever present.** On both the MCP and sync push update paths, and for both tasks and artifacts, `expected_status` is a claim about the row's current state and is enforced whenever the caller supplies it — regardless of whether the payload itself mutates status. A title-only update that carries `expected_status="draft"` while the row is now `ready` returns `STATUS_CONFLICT`. Rationale: an `expected_status` in a precondition is the client asserting "I believe the row looks like X"; the truthful response to a mismatch is a conflict, not a silent stale-write. This is the stricter of the two possible semantics and matches every code path in the server — do NOT reintroduce the "only enforce when status changes" split.
