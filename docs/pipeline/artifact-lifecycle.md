...
`metadata.claimed_by` is a useful audit-trail supplement (which interceptor holds the claim, when it started) but is not required to detect the race — `STATUS_CONFLICT` is the primary signal. See the claim pseudocode in [build-your-own-interceptor.md](build-your-own-interceptor.md).
